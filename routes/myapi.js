...
router.get('/feed', (req, res, next) => {
	Post.getPostsFeed((err, feed) => {
		if (err) {
			res.json({ success: false, msg: 'Failed to retrieve feed' });
		} else {
			res.json(feed);
		}
	});
...
router.get('/post/:id', (req, res, next) => {
	...
	Post.getPost(id, (err, post) => {
		if (err) {
			res.json({ success: false, msg: 'Failed to retrieve post' });
		} else {
			res.json(post);
		}
	});
...
// post
router.post('/post/submit', passport.authenticate('jwt', { session: false }), (req, res, next) => {
...
router.put('/post/:id/upvote', passport.authenticate('jwt', { session: false }), (req, res, next) => {
	...
	Post.votePost(id, 1, (err, post) => {
		if (err) {
			res.json({ success: false, msg: 'Failed to vote' });
		} else {
			res.json({ success: true, msg: 'Upvoted' });
		}
	});
});
...
router.put('/post/:id/downvote', passport.authenticate('jwt', { session: false }), (req, res, next) => {
	...
	Post.votePost(id, -1, (err, post) => {
		if (err) {
			...
			res.json({ success: true, msg: 'Downvoted' });
		}
	});
});
